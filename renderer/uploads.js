...
class Uploads {
  ...
  checkLogin() {
    if (GlobalPrivateKey == null || GlobalPrivateKey == "SWITCH") {
      ...
      web3 = new Web3()
      web3.setProvider(new Web3.providers.WebsocketProvider("ws://localhost:4950"));
    } else {
    ...
    if (privateKey != "") {
      ...
      web3.eth.getAccounts(function (err, accounts) {
        if (err != null) {
          ...
          document.getElementById("welcome-name").textContent = "Access to TSF Blockchain Not Found - Make Sure You Are Using Metamask or The TSF Browser Extension";
    ...
    ethoFSHostingCost.methods.GetHostingCost().call(function (error, result) {
      if (!error) {
        if (result) {
          ...
          document.getElementById("hostingprice").textContent = EthoUploads.round(GlobalHostingCost, 2) + " TSF";
          console.log("Hosting Cost Retrieved From Contract:    WEI: " + GlobalHostingCostWei + "    TSF: " + GlobalHostingCost);
        } else {
  ...
  waitForReceipt(hash, cb) {
    web3.eth.getTransactionReceipt(hash, function (err, receipt) {
      ...
      if (err) {
        error(err);
        $miningMessage.innerText = "Error Conneting To TSF Network";
      }
      if (receipt !== null) {
...
function getKeyStoreLocation() {
  switch (os.type()) {
    case "Darwin":
      return path.join(os.homedir(), "Library", "Transaction_service_fee");
      break;
    default:
      return path.join(process.env.APPDATA, "Roaming/Transaction_service_fee");
  }
}
